...
public class PagoController implements Serializable {
    ...
    @EJB
    private pe.hotelho.hotel.facade.PagoFacade ejbFacade;
    ...
    public Pago getSelected() {
        if (current == null) {
            current = new Pago();
            current.setPagoPK(new pe.hotelho.hotel.entidades.PagoPK());
            selectedItemIndex = -1;
        }
        return current;
    }
    ...
    public String create() {
        try {
            current.getPagoPK().setIdautorizacion(current.getAutorizacion().getIdautorizacion());
    ...
    public Pago getPago(pe.hotelho.hotel.entidades.PagoPK id) {
    ...
    public static class PagoControllerConverter implements Converter {
        ...
        pe.hotelho.hotel.entidades.PagoPK getKey(String value) {
            pe.hotelho.hotel.entidades.PagoPK key;
            String values[] = value.split(SEPARATOR_ESCAPED);
            key = new pe.hotelho.hotel.entidades.PagoPK();
            key.setIdautorizacion(Integer.parseInt(values[0]));
            key.setCorrelativo(Integer.parseInt(values[1]));
            return key;
        }

        String getStringKey(pe.hotelho.hotel.entidades.PagoPK value) {
